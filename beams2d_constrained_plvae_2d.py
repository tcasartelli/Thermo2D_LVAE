...
"""Constrained Performance-LVAE for beams2D designs with plummet-based dynamic pruning.
...
problem_id and resize_dimensions set for beams2d dataset (50x100 designs).
...
@dataclass
class Args:
    """Command-line arguments for constrained Performance-LVAE training on beams2d."""

    # Problem and tracking
    problem_id: str = "beams2d"
    """Problem ID — beams2d has 500x100 designs."""
    ...
    images_dir: str = os.path.join(os.environ.get("SCRATCH", "."), "beams2d_constrained_plvae", "images")
    """Directory to save visualisation images."""
    checkpoint_dir: str = os.path.join(os.environ.get("SCRATCH", "."), "beams2d_constrained_plvae", "checkpoints")
...
if __name__ == "__main__":
    ...
    design_shape = problem.design_space.shape  # (50, 100)
    ...
    print(f"\n{'=' * 60}")
    print("Constrained Performance-LVAE — beams2d")
    ...
    x_train = train_ds["optimal_design"][:].unsqueeze(1)  # (N, 1, 50, 100)
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.save_model and epoch == args.n_epochs - 1:
            ckpt_path = os.path.join(args.checkpoint_dir, "beams2d_constrained_plvae.pth")
            th.save({
                "epoch"    : epoch,
