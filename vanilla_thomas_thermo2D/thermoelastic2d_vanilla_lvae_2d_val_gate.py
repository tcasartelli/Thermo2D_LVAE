"""Vanilla LVAE for thermoelastic2D — GN encoder + validation-gated volume loss.

Volume pressure and pruning activate only when BOTH train_nmse AND val_nmse are
below the volume threshold. This prevents latent space compression before the
model has generalized, reducing the train/val NMSE gap.

Note: vanilla LVAE uses a fixed volume weight (no NMSE threshold), so val-gate
here blocks volume when val_rec > nmse_val_threshold (a separate parameter).
...
@dataclass
class Args:
    """Command-line arguments for vanilla LVAE with val-gate training on thermoelastic2d."""
    ...
    """NMSE threshold for both train and val before volume activates."""
    ...
    images_dir: str = os.path.join(os.environ.get("SCRATCH", "."), "thermoelastic2d_lvae_val_gate", "images")
    checkpoint_dir: str = os.path.join(os.environ.get("SCRATCH", "."), "thermoelastic2d_lvae_val_gate", "checkpoints")
...
if __name__ == "__main__":
    ...
    # Use ConstrainedLeastVolumeAE_DP with val_gate — it naturally supports the
    # NMSE-gated + val-gated pattern we want.
    ...
    print(f"\n{'=' * 60}")
    print("Vanilla LVAE (constrained) — thermoelastic2d [GN encoder + val-gate]")
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.save_model and epoch == args.n_epochs - 1:
            ckpt_path = os.path.join(args.checkpoint_dir, "thermoelastic2d_lvae_val_gate.pth")
            th.save({
                "epoch"    : epoch,
