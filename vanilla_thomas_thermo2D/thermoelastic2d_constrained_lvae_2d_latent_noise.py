"""Constrained LVAE for thermoelastic2D designs — GN encoder + latent noise injection.
...
Clean z is still used for pruning statistics and volume loss computation.
...
@dataclass
class Args:
    """Command-line arguments for constrained LVAE with latent noise training on thermoelastic2d."""
    ...
    images_dir: str = os.path.join(os.environ.get("SCRATCH", "."), "thermoelastic2d_constrained_lvae_latent_noise", "images")
    """Directory to save visualisation images."""
    checkpoint_dir: str = os.path.join(os.environ.get("SCRATCH", "."), "thermoelastic2d_constrained_lvae_latent_noise", "checkpoints")
...
if __name__ == "__main__":
    ...
    # ---- Constrained LVAE with latent noise ----
    ...
    print(f"\n{'=' * 60}")
    print("Constrained LVAE — thermoelastic2d [GN encoder + latent noise]")
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.save_model and epoch == args.n_epochs - 1:
            ckpt_path = os.path.join(args.checkpoint_dir, "thermoelastic2d_constrained_lvae_latent_noise.pth")
            th.save({
                "epoch"    : epoch,
